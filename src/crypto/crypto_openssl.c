...
/* Binary data for the curve parameters - subgroup of order 269 */
static unsigned char big_b_bin[] = {0xfe, 0x52, 0x40, 0xb6, 0xee, 0x72, 0x73, 0xd1, 0x43, 0x09, 0xcc, 0x22, 0xf0, 0x66, 0x4a, 0xd4, 0x5a, 0xd7, 0x6d, 0x4b, 0x13, 0x89, 0x95, 0x2c, 0x3f, 0xd9, 0xf8, 0x54, 0xd5, 0x5b, 0xfa, 0x80};
static unsigned char big_order_bin[] = {0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0x0b, 0x2e, 0x09, 0x5b, 0x89, 0x8d, 0xf5, 0xb3, 0xf5, 0x9b, 0x25, 0xb0, 0x68, 0xb3, 0xa4, 0x7d};
static unsigned char big_x_bin[] = {0x6b, 0xfe, 0x1a, 0x57, 0x02, 0x46, 0x8a, 0xc7, 0xe2, 0xe7, 0xef, 0xd2, 0x2a, 0x25, 0xcc, 0xf5, 0x1a, 0xda, 0xbf, 0x60, 0xd3, 0x85, 0x17, 0x33, 0x2f, 0x07, 0xd2, 0x2f, 0x9a, 0x88, 0x75, 0x55};
static unsigned char big_y_bin[] = {0x75, 0x82, 0xb0, 0x15, 0x15, 0x68, 0x89, 0xef, 0x88, 0xe5, 0x03, 0x05, 0x4b, 0xd9, 0x71, 0x75, 0xdf, 0xb6, 0x19, 0x7c, 0x94, 0x32, 0xa4, 0xfc, 0x6c, 0xfb, 0x07, 0x5e, 0xcb, 0x2d, 0xb9, 0x2d};

/* Binary data for the curve parameters - subgroup of order 3 */
static unsigned char small_b_bin[] = {0x45, 0xff, 0x81, 0xaa, 0xca, 0x78, 0x93, 0x3d, 0x79, 0xd0, 0x74, 0x48, 0x11, 0xb1, 0xd9, 0x0e, 0xc3, 0x88, 0xc0, 0xa5, 0x18, 0xc2, 0xa7, 0x61, 0xc1, 0x3e, 0x67, 0x9e, 0xd0, 0xc7, 0x75, 0x43};
static unsigned char small_order_bin[] = {0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x73, 0x57, 0x08, 0x1e, 0xb4, 0x4a, 0x87, 0xfc, 0x53, 0x84, 0x03, 0xef, 0x52, 0x55, 0xd4, 0x0d};
static unsigned char small_x_bin[] = {0x5a, 0x9a, 0x06, 0xc4, 0x9c, 0x61, 0x8e, 0x65, 0x97, 0x37, 0x99, 0x29, 0x2c, 0xf4, 0xf9, 0x0a, 0x82, 0xb2, 0x86, 0x00, 0x65, 0xcf, 0x2d, 0xe3, 0x0f, 0xb4, 0x07, 0x6d, 0xd5, 0xb8, 0xd2, 0x8d};
static unsigned char small_y_bin[] = {0xbd, 0x06, 0x5f, 0x28, 0x19, 0xe3, 0xe2, 0x5e, 0xe3, 0x89, 0x2f, 0x04, 0x77, 0x5f, 0xfd, 0xde, 0xb4, 0x69, 0x2a, 0x2d, 0xcc, 0x7b, 0xa5, 0x20, 0x5c, 0xe7, 0x29, 0xa9, 0xb0, 0x5b, 0xea, 0xd2};

#define PARAMS_SIZE sizeof(p_bin)

static void get_subgroup_params(int size, unsigned char **b_bin, unsigned char **order_bin,
				unsigned char **x_bin, unsigned char **y_bin)
{
	if (size == 3) {
		*b_bin = small_b_bin;
		*order_bin = small_order_bin;
		*x_bin = small_x_bin;
		*y_bin = small_y_bin;
	} else {
		*b_bin = big_b_bin;
		*order_bin = big_order_bin;
		*x_bin = big_x_bin;
		*y_bin = big_y_bin;
	}
}
...
struct crypto_ec * crypto_ec_subgroup(int size, struct crypto_ec_point **out_generator)
{
	...
	EC_POINT *generator;
	struct crypto_ec *e;
	unsigned char *b_bin, *order_bin, *x_bin, *y_bin;
	...
	get_subgroup_params(size, &b_bin, &order_bin, &x_bin, &y_bin);

	/* Set the values for the various parameters */
	e->a = BN_bin2bn(a_bin, PARAMS_SIZE, NULL);
	e->b = BN_bin2bn(b_bin, PARAMS_SIZE, NULL);
	e->prime = BN_bin2bn(p_bin, PARAMS_SIZE, NULL);
	e->order = BN_bin2bn(order_bin, PARAMS_SIZE, NULL);
	e->bnctx = BN_CTX_new();
	x = BN_bin2bn(x_bin, PARAMS_SIZE, NULL);
	y = BN_bin2bn(y_bin, PARAMS_SIZE, NULL);
	if (e->a == NULL || e->b == NULL || e->prime == NULL || e->order == NULL ||
	    e->bnctx == NULL || x == NULL || y == NULL) {
		printf("%s: failed setting parameters\n", __FUNCTION__);
		BN_free(x);
		BN_free(y);
		crypto_ec_deinit(e);
		return NULL;
	...
	/* Create the curve */
	e->group = EC_GROUP_new_curve_GFp(e->prime, e->a, e->b, e->bnctx);
	if(e->group == NULL) {
		BN_free(x);
		BN_free(y);
		crypto_ec_deinit(e);
	...
	generator = EC_POINT_new(e->group);
	if(1 != EC_POINT_set_affine_coordinates_GFp(e->group, generator, x, y, e->bnctx)) {
		BN_free(x);
		BN_free(y);
		crypto_ec_deinit(e);
		return NULL;
	...
	if(1 != EC_GROUP_set_generator(e->group, generator, e->order, NULL)) {
		BN_free(x);
		BN_free(y);
		crypto_ec_deinit(e);
		return NULL;
	...
	*out_generator = (struct crypto_ec_point *)generator;
...
struct crypto_ec_point * crypto_ec_point_get_zeroscalar_element(struct crypto_ec *e)
{
	uint8_t point_data[] =  {
		// X coordinate
		0xe6, 0x65, 0xba, 0x8d, 0x4b, 0x6a, 0x4d, 0x4c, 0x32, 0xda, 0x01, 0xce, 0xa1, 0x52, 0xb9, 0xb3,
		0x08, 0x09, 0xde, 0xcf, 0xaa, 0x2b, 0x15, 0xb0, 0xab, 0xb1, 0x58, 0x2f, 0xc5, 0x5b, 0xd7, 0xc8,
		0x42, 0x1c, 0xbd, 0xd9, 0x2e, 0x0f, 0x9b, 0x34, 0x63, 0x81, 0xed, 0xa5, 0x46, 0xa4, 0x0e, 0x4f,
		// Y coordinate
		0x2a, 0xab, 0x95, 0x6c, 0x0a, 0x80, 0xb3, 0x50, 0x54, 0x2c, 0xa9, 0x46, 0xfc, 0x9f, 0xe8, 0xc8,
		0x6b, 0x21, 0xfe, 0x0a, 0x63, 0xe6, 0xa4, 0x6b, 0xa6, 0xb3, 0x35, 0x62, 0xe7, 0x32, 0xde, 0x70,
		0x45, 0xbd, 0xb8, 0x94, 0x22, 0x4b, 0xdf, 0xac, 0x59, 0xa5, 0xa6, 0xed, 0xb6, 0xfe, 0x4c, 0x5a
	};

	/** TODO: Better error handling */
	int len = BN_num_bytes(e->prime);
	if (2 * len != sizeof(point_data))
		return NULL;

	return crypto_ec_point_from_bin(e, point_data);
}
#endif // DRAGONSLAYER
